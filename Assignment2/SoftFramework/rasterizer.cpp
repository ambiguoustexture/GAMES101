...
{   
    ...
    Eigen::Vector2f side0            = _v[1].head(2) - _v[0].head(2),
                    side1            = _v[2].head(2) - _v[1].head(2),
                    side2            = _v[0].head(2) - _v[2].head(2);

    Eigen::Vector2f vertex0_to_point = point         - _v[0].head(2),
                    vertex1_to_point = point         - _v[1].head(2),
                    vertex2_to_point = point         - _v[2].head(2);
    
    return side0[0] * vertex0_to_point[1] - side0[1] * vertex0_to_point[0] > 0
        && side1[0] * vertex1_to_point[1] - side1[1] * vertex1_to_point[0] > 0
        && side2[0] * vertex2_to_point[1] - side2[1] * vertex2_to_point[0] > 0;
}
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) 
{
    ...
    float box_t = (int)std::ceil (std::max(v[0][1], std::max(v[1][1], v[2][1]))),
          ...
          box_l = (int)std::floor(std::min(v[0][0], std::min(v[1][0], v[2][0]))),
          box_r = (int)std::ceil (std::max(v[0][0], std::max(v[1][0], v[2][0])));
        ...
        for (int x = box_l; x <= box_r; x++) 
        {
            for (int y = box_b; y <= box_t; y++) 
            {
                ...
                if (cnt != 0) 
                {
                    ...
                    {
                        Vector3f color = t.getColor() * cnt / 4;
                        Vector3f point(3);
